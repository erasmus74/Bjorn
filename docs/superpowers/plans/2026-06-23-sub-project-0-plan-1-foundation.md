...
def repo(tmp_path):
    ...
    conn = factory.connect()
    factory.apply_schema(conn)
    return ActionLogRepository(conn)


def test_insert_assigns_id(repo):
    entry = repo.insert(
        global_mode="active",
...
def test_insert_with_full_fields(repo):
    repo.insert(
        global_mode="active",
        scope_basis="operator-confirmed-active-mode",
...
def test_list_recent_filter_by_network(repo):
    repo.insert("active", "x", "a.started", target_network_id=1, outcome="started")
    repo.insert("active", "x", "b.started", target_network_id=2, outcome="started")
    result = repo.list_for_network(network_id=1, limit=10)
    assert len(result) == 1
    assert result[0]["target_network_id"] == 1
